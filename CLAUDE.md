...
├── cli.py              — experimental Textual TUI (learning project): login → MainScreen, a single combined view — job list, a persistent horizontal filter bar between list and detail, and a live detail panel for the highlighted row. List/detail borders switch color based on which has keyboard focus. `j`/`k`/arrows: in the list, moves the row cursor; in the detail panel (the third Tab stop), moves between layer sections (summary → Triage → ... → Gut feeling → wraps to summary), highlighting and scrolling the current one into view — the summary is the default landing spot. Filter bar: `Tab`/`←`/`→` cycle the 8 statuses (shown with VIM-style key indicators), or jump directly via quick-select keys (`a`/`r`/`g`/`w`/`p`/`i`/`o`/`c`); scrolls the selection into view on narrow terminals. `/` opens live text search, `:` opens a command line (`analyze <url|text>`, `filter <status>`, `full`/`brief`, `quit`) — both share one input. `:analyze` runs scrape→analyze→save in a worker thread (`exclusive=True`, guards against a concurrent second API call), status inline, duplicate confirmation typed (`yes`/anything-else). `?` (and Settings' "Show keys") opens Textual's built-in dockable help panel, auto-listing whatever's bound on the focused widget. `Ctrl+S` opens Settings (replaces Textual's default command palette): edit CV/Zero list/Yellow list/criteria in a full-screen text editor, toggle light/dark theme, show keybindings, save screenshot, quit. `Ctrl+P` saves a screenshot from any screen. Filter/layer-view/help-panel-visibility state persists per-user across sessions (`data/cli_state_<username>.json`, gitignored). Wraps database.py/analyzer.py/scraper.py directly, no HTTP. Run via `uv run --env-file config.env python cli.py`. Theme matches CLI/terminal.html's mockup palette (not committed elsewhere in the app's CSS). Not a documented production entry point — separate from the Flask app's endpoints below.
