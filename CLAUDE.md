...
├── cli.py              — experimental Textual TUI (learning project): login → MainScreen, a single combined view — job list, a persistent horizontal filter bar between list and detail, and a live detail panel for the highlighted row showing all six layers plus Fit and Gut feeling at once (no brief/full toggle). List/detail borders switch color based on which has keyboard focus. `j`/`k`/arrows: in the list, moves the row cursor; in the detail panel (the third Tab stop), moves between layer sections (summary → Triage → ... → Gut feeling → wraps to summary), highlighting and scrolling the current one into view — the summary is the default landing spot. Filter bar: `Tab`/`←`/`→` cycle the 8 statuses (shown with VIM-style key indicators — the bound letter highlighted in reverse video within the word itself), or jump directly via quick-select keys (`a`/`r`/`g`/`w`/`p`/`i`/`o`/`c`); scrolls the selection into view on narrow terminals. `/` and `:` open the same merged prompt — live-filters the job list by role/company substring on every keystroke; on Enter, recognized commands (`analyze <url|text>`, `filter <status>`, `quit`/`q`) run and clear the filter, anything else stays applied as the filter. `:analyze` runs scrape→analyze→save in a worker thread (`exclusive=True`, guards against a concurrent second API call), status inline, duplicate confirmation typed (`yes`/anything-else). `?` (and Settings' "Show keys") opens Textual's built-in dockable help panel, auto-listing whatever's bound on the focused widget, sized to fit the longest line on any screen without wrapping. `Ctrl+S` opens "Settings" (Textual's default command palette, retitled, with its full built-in theme picker preserved — not replaced): edit CV/Zero list/Yellow list/criteria in a full-screen text editor, change theme (dark/light plus every Textual built-in), show keybindings, save screenshot, quit. `Ctrl+P` saves a screenshot from any screen. A single bottom key bar (Textual's `Footer`, no separate legend row) shows every binding with `|`-separated, reverse-block-styled key letters. Filter/layer-view/help-panel-visibility state persists per-user across sessions (`data/cli_state_<username>.json`, gitignored). Wraps database.py/analyzer.py/scraper.py directly, no HTTP. Run via `uv run --env-file config.env python cli.py`. Theme matches CLI/terminal.html's mockup palette (not committed elsewhere in the app's CSS). Not a documented production entry point — separate from the Flask app's endpoints below.
