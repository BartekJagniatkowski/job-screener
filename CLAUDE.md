...
├── cli.py              — experimental Textual TUI (learning project): login → MainScreen, a single combined view (job list + live detail panel for the highlighted row, status filter cycling, `j`/`k`/arrow navigation). `/` opens live text search, `:` opens a command line (`analyze <url|text>`, `filter <status>`, `full`/`brief`, `quit`) — both share one input. `:analyze` runs scrape→analyze→save in a worker thread (`exclusive=True`, guards against a concurrent second API call), status inline, duplicate confirmation typed (`yes`/anything-else). `Ctrl+S` opens Settings (replaces Textual's default command palette): edit CV/Zero list/Yellow list/criteria in a full-screen text editor, toggle light/dark theme, show keybindings, save screenshot, quit. Wraps database.py/analyzer.py/scraper.py directly, no HTTP. Run via `uv run --env-file config.env python cli.py`. Theme matches CLI/terminal.html's mockup palette (not committed elsewhere in the app's CSS). Not a documented production entry point — separate from the Flask app's endpoints below.
